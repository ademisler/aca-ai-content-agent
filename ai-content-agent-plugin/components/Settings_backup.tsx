...
export const Settings: React.FC<SettingsProps> = ({ settings, onSaveSettings, onRefreshApp, onShowToast, openSection }) => {
    ...
    const getTabContent = (tabId: string): React.ReactNode => {
        // We'll return the content based on the active tab
        // For now, we'll show all sections and hide them based on activeTab
        return (
            <div>
                {tabId === 'license' && renderLicenseContent()}
                {tabId === 'automation' && renderAutomationContent()}
                {tabId === 'integrations' && renderIntegrationsContent()}
                {tabId === 'content' && renderContentContent()}
                {tabId === 'advanced' && renderAdvancedContent()}
            </div>
        );
    };
