...
class ACA_RankMath_Compatibility {
    ...
    public function init() {
        add_filter('aca_content_generation_context', [$this, 'add_rankmath_context']);
        add_action('aca_content_generated', [$this, 'update_rankmath_meta'], 10, 2);
        add_filter('aca_seo_title', [$this, 'optimize_for_rankmath'], 10, 2);
        add_filter('aca_meta_description', [$this, 'optimize_meta_for_rankmath'], 10, 2);
    }
    
    public function add_rankmath_context($context) {
        $context['seo_plugin'] = 'rankmath';
        $context['focus_keywords'] = $this->get_focus_keywords();
        $context['seo_score_target'] = 80; // RankMath good score threshold
        return $context;
    }
    
    public function update_rankmath_meta($post_id, $content_data) {
        if (!function_exists('rank_math')) {
            return;
        }
        
        // Update RankMath meta fields
        if (isset($content_data['seo_title'])) {
            update_post_meta($post_id, 'rank_math_title', sanitize_text_field($content_data['seo_title']));
        }
        
        if (isset($content_data['meta_description'])) {
            update_post_meta($post_id, 'rank_math_description', sanitize_textarea_field($content_data['meta_description']));
        }
        ...
        // Set schema type if detected
        if (isset($content_data['schema_type'])) {
            update_post_meta($post_id, 'rank_math_rich_snippet', sanitize_text_field($content_data['schema_type']));
        }
    }
    
    public function optimize_for_rankmath($title, $post_id) {
        if (!function_exists('rank_math')) {
            return $title;
        }
        
        $focus_keyword = get_post_meta($post_id, 'rank_math_focus_keyword', true);
        if ($focus_keyword && strpos(strtolower($title), strtolower($focus_keyword)) === false) {
    ...
    public function optimize_meta_for_rankmath($description, $post_id) {
        if (!function_exists('rank_math')) {
            return $description;
        }
        
        $focus_keyword = get_post_meta($post_id, 'rank_math_focus_keyword', true);
        if ($focus_keyword && strpos(strtolower($description), strtolower($focus_keyword)) === false) {
            // Naturally incorporate focus keyword in description
            $description = str_replace('.', '. ' . $focus_keyword, $description, 1);
        }
        
        return $description;
    }
    
    private function get_focus_keywords() {
        // Get common focus keywords from recent posts
        global $wpdb;
