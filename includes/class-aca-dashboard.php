...
class ACA_Dashboard {
    ...
    private static function render_overview_section() {
        ...
        echo '<p>' . sprintf( esc_html__( 'API Usage: %1$s / %2$s calls this month.', 'aca' ), number_format_i18n( $api_usage ), $api_limit > 0 ? number_format_i18n( $api_limit ) : esc_html__( 'unlimited', 'aca' ) ) . '</p>';
        /* translators: %s: number of pending ideas */
        echo '<p>' . sprintf( esc_html__( 'Pending Ideas: %s', 'aca' ), number_format_i18n( $pending_ideas ) ) . '</p>';
        /* translators: %s: number of drafted posts */
        echo '<p>' . sprintf( esc_html__( 'Drafted Posts: %s', 'aca' ), number_format_i18n( $drafted_posts ) ) . '</p>';
    }
        ...
        if (!empty($ideas)) {
            ...
            foreach ($ideas as $idea) {
                echo '<li data-id="' . esc_attr( $idea->id ) . '">' . esc_html( $idea->idea_title ) .
                     ' <button class="button-primary aca-write-draft" data-id="' . esc_attr( $idea->id ) . '">' . esc_html__( 'Write Draft', 'aca' ) . '</button>' .
                     ' <span class="aca-draft-status"></span>' .
                     ' <button class="button-secondary aca-reject-idea" data-id="' . esc_attr( $idea->id ) . '">' . esc_html__( 'Reject', 'aca' ) . '</button>' .
